...
BYTE* dir /* Pointer to the directory entry */
  ...
  if (fs->fs_type == FS_FAT32) cl |= (DWORD) LD_WORD(dir + DIR_FstClusHI) << 16;
...
static BYTE
check_fs( /* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
FATFS* fs, /* File system object */
...
void* obj /* Pointer to the object FIL/DIR to check validity */
  ...
  if (!fil || !fil->fs || !fil->fs->fs_type || fil->fs->id != fil->id) return FR_INVALID_OBJECT;

  ENTER_FF(fil->fs); /* Lock file system */

  if (disk_status(fil->fs->drv) & STA_NOINIT) return FR_NOT_READY;
...
#else
        if ((fp->flag & FA__DIRTY) && fp->dsect - sect < cc) mem_cpy(rbuff + ((fp->dsect - sect) * SS(fp->fs)), fp->buf, SS(fp->fs));
#endif
#endif
...
f_unlink(const TCHAR* path /* Pointer to the file or directory path */
  ...
  if (res == FR_OK) {
    INIT_BUF(dj);
    res = follow_path(&dj, path); /* Follow the file path */
    if (_FS_RPATH && res == FR_OK && (dj.fn[NSFLAG] & NS_DOT)) res = FR_INVALID_NAME; /* Cannot remove dot entry */
#if _FS_LOCK
    ...
    if (res == FR_OK) { /* The object is accessible */
      ...
      if (!dir) {
        ...
        if (dir[DIR_Attr] & AM_RDO) res = FR_DENIED; /* Cannot remove R/O object */
      }
      if (res == FR_OK) {
...
f_mkdir(const TCHAR* path /* Pointer to the directory path */
  ...
  if (res == FR_OK) {
    ...
    if (_FS_RPATH && res == FR_NO_FILE && (dj.fn[NSFLAG] & NS_DOT)) res = FR_INVALID_NAME;
    if (res == FR_NO_FILE) { /* Can create a new directory */
      ...
      if (res == FR_OK) { /* Initialize the new directory table */
        ...
        dir[SZ_DIRE + 1] = '.';
        pcl = dj.sclust;
        if (dj.fs->fs_type == FS_FAT32 && pcl == dj.fs->dirbase) pcl = 0;
...
BYTE mask /* Attribute mask to change */
  ...
  if (res == FR_OK) {
    ...
    res = follow_path(&dj, path); /* Follow the file path */
    FREE_BUF();
    if (_FS_RPATH && res == FR_OK && (dj.fn[NSFLAG] & NS_DOT)) res = FR_INVALID_NAME;
    if (res == FR_OK) {
      dir = dj.dir;
...
const TCHAR* path_new /* Pointer to the new name */
  ...
  if (res == FR_OK) {
    ...
    if (_FS_RPATH && res == FR_OK && (djo.fn[NSFLAG] & NS_DOT)) res = FR_INVALID_NAME;
#if _FS_LOCK
        ...
        if (res == FR_NO_FILE) { /* It is a valid path and no name collision */
          ...
          if (res == FR_OK) {
            ...
            if (res == FR_OK) {
              ...
              if (res == FR_OK) res = sync_fs(djo.fs);
            }
...
UINT au /* Size of allocation unit in unit of byte or sector */
  ...
  if ((fmt == FS_FAT16 && n_clst < MIN_FAT16) || (fmt == FS_FAT32 && n_clst < MIN_FAT32)) return FR_MKFS_ABORTED;
  ...
  for (i = 0; i < N_FATS; i++) { /* Initialize each FAT copy */
    ...
    if (disk_write(pdrv, tbl, wsect++, 1) != RES_OK) return FR_DISK_ERR;
...
FIL* fp /* Pointer to the file object */
  ...
  && f_write(pb.fp, pb.buf, (UINT) pb.idx, &nw) == FR_OK && (UINT) pb.idx == nw) return pb.nchr;
  return EOF;
}

...
static BYTE wait_ready(void) {
  ...
  clock_t start = disk_time();
  rcvr_spi();
  do
    res = rcvr_spi();
  while ((res != 0xFF) && ((disk_time() - start) <= 500)); /* Wait for ready in timeout of 500ms */
...
bool rcvr_datablock(BYTE *buff, /* Data buffer to store received data */
  ...
  clock_t start = disk_time();
  do { /* Wait for data packet in timeout of 1s */
    token = rcvr_spi();
  } while ((token == 0xFF) && ((disk_time() - start) <= 1000));
...
DSTATUS disk_initialize(BYTE drv /* Physical drive number (0) */
  ...
  if (send_cmd(CMD0, 0) == 1) { /* Enter Idle state */
    clock_t start = disk_time(), current = start;
    if (send_cmd(CMD8, 0x1AA) == 1) { /* SDC Ver2+ */
      ...
      if (ocr[2] == 0x01 && ocr[3] == 0xAA) { /* The card can work at vdd range of 2.7-3.6V */
        ...
        } while (((current = disk_time()) - start) <= 1000);
        if (((current - start) <= 1000) && send_cmd(CMD58, 0) == 0) { /* Check CCS bit */
          for (n = 0; n < 4; n++)
            ocr[n] = rcvr_spi();
          ty = (ocr[0] & 0x40) ? 6 : 2;
        }
      ...
      if (((current - start) > 1000) || send_cmd(CMD16, 512) != 0) /* Select R/W block length */
      ty = 0;
    }
  }
  CardType = ty;
...
DRESULT disk_write(BYTE drv, /* Physical drive number (0) */
  ...
  if (count == 1) { /* Single block write */
    if ((send_cmd(CMD24, sector) == 0) /* WRITE_BLOCK */
        && xmit_datablock(buff, 0xFE)) count = 0;

...
gyroCalibrate(uint32_t ui32SampleCount, uint32_t ui32SigmaMultiple) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    gyro_fThreshold[i] = sqrt(
        (fSigma[i] / (float) ui32SampleCount) - pow(gyro_fZeroRate[i], 2))
        * (float) ui32SigmaMultiple;
  }
}
void
gyroInit(uint32_t ui32Base, uint8_t ui8GyroAddr, bool bSpeed) {
  ...
  do {  // Loop until all initialization calls complete successfully
    // 1. Test if device is there
    if (!gyroTest()) continue;
    // 2. Write CTRL_REG2 (HPF Behavior, Cutoff Freq)
    if (!gyroConfigHPF(GYRO_NORMAL_MODE, 0x0)) continue;
    // 3. Write CTRL_REG3 (Interrupts)
    // if (!gyroConfigInterrupts()) continue;
    // 4. Write CTRL_REG4 (Data Config, Self test, SPI)
    if (!gyroDataConfig(GYRO_BATCH_UPDATE, GYRO_DPS250)) continue;
    // 5. Write CTRL_REG5 (Boot Mode, FIFO Behavior, HPF Enable, Int1 Selection, Out Selection)
    if (!gyroBoot(GYRO_BOOT_NORMAL)) continue;
    // 6. Write CTRL_REG1 (Data rate, Bandwith)
    if (!gyroPowerOn(GYRO_ODR400FC110)) continue;
    break;
  } while (true);
...
gyroPowerOn(uint32_t ui32odr) {
  uint8_t ui8Data[2] = { GYRO_CTRL_REG1, ui32odr | GYRO_NORMAL | GYRO_ZEN
      | GYRO_YEN | GYRO_XEN };
  if (!I2CBurstWrite(gyro_ui32Base, gyro_ui8GyroAddr, &ui8Data[0], 2))
    return false;
  return true;
}
bool
...
gyroDataConfig(uint32_t ui32upd, uint32_t ui32res) {
  uint8_t ui8Data[2] =
      { GYRO_CTRL_REG4, ui32upd | GYRO_LITTLE_ENDIAN | ui32res };
  if (!I2CBurstWrite(gyro_ui32Base, gyro_ui8GyroAddr, &ui8Data[0], 2))
    return false;
  switch (ui32res) {
    case 0x00 << 4: {
...
gyroReadXYZRaw(int16_t* i16Raw) {
  uint8_t ui8Data[6] = { 0 }, ui8Status = 0x0, i;
  if (!(gyroStatus(&ui8Status))) return false;
  if ((ui8Status & GYRO_ZYXDA) != GYRO_ZYXDA) return false;  // No new data is available
  if (!I2CWrite(gyro_ui32Base, gyro_ui8GyroAddr,
      GYRO_OUT_X_L | GYRO_AUTO_INCREMENT)) return false;
  if (!I2CBurstRead(gyro_ui32Base, gyro_ui8GyroAddr, &ui8Data[0], 6))
    return false;
  for (i = 0; i < 3; i++) {
